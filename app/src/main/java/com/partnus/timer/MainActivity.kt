...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        progressBar3.setInitTime(10000L) // 초기 시간 설정 10초
        CoroutineScope(Dispatchers.Main).launch {
            flow {
                // 10초 부터 0초까지 1초 간격으로 flow 전송
                for (currentPercent in progressBar3.getInitTime() downTo 0 step 1000) {
                    emit(currentPercent)
                    delay(1000L)
                }
            }.collect { percent ->
                progressBar3.setCurrentTime(percent) // 현재 시간 progressbar에 적용
            }
        }
    }
}
